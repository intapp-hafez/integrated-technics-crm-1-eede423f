...
export async function runSecurityScan(): Promise<{
  ...
  findings.push({
    ...
    remediation:
      "Never render unescaped query strings. React renders text nodes safely by default.",
  });
  ...
  const sqlLeak =
    sqlProbe.ok && /postgres|syntax error at or near|pg_|SQLSTATE/i.test(sqlProbe.body);
  findings.push({
    id: "sqli.error-leak",
    category: "injection",
    owasp: "A03:2025",
  ...
  findings.push({
    ...
    status: "info",
    severity: "low",
    evidence: adminProbe.ok
      ? `HTTP ${adminStatus} (SPA returns 200 — gating is client-side)`
      : (adminProbe as any).error,
  ...
  try {
    ...
    findings.push({
      ...
      category: "rls",
      owasp: "A01:2025",
      title: "Could not verify DB connectivity",
      status: "warn",
      severity: "medium",
      evidence: e?.message ?? "",
      remediation: "Check Supabase configuration.",
    });
  }
  ...
  const isLocalhost =
    origin.includes("localhost") || origin.includes("127.0.0.1") || origin.includes("10.10.");
  findings.push({
    id: "tls.https",
    category: "config",
    owasp: "A02:2025",
  ...
  const cookieAudit: ScanStatus = setCookies
    ? /secure/i.test(setCookies) && /samesite/i.test(setCookies)
      ? "pass"
      : "warn"
    : "info";
  findings.push({
    ...
    status: cookieAudit,
    severity: cookieAudit === "warn" ? "medium" : "info",
