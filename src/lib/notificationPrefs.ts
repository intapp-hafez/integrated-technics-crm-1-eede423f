...
export const NOTIF_CATEGORIES: {
  ...
  { key: "lead", labelEn: "Leads", labelAr: "العملاء المحتملون", descEn: "New leads and status changes", descAr: "العملاء الجدد وتغييرات الحالة", roles: ["admin", "manager", "finance", "employee"] },
  { key: "project", labelEn: "Projects", labelAr: "المشاريع", descEn: "Project creation, status and updates", descAr: "إنشاء وتحديث وحالة المشاريع", roles: ["admin", "manager", "finance", "employee"] },
  { key: "chat", labelEn: "Chat Messages", labelAr: "رسائل الدردشة", descEn: "New messages and replies", descAr: "الرسائل والردود الجديدة", roles: ["admin", "manager", "finance", "hr", "employee"] },
  { key: "activities", labelEn: "Activities", labelAr: "الأنشطة", descEn: "New activities and changes", descAr: "الأنشطة الجديدة والتغييرات", roles: ["admin", "manager", "employee"] },
  { key: "attendance", labelEn: "Attendance", labelAr: "الحضور", descEn: "Check-in / check-out alerts", descAr: "تنبيهات الحضور والانصراف", roles: ["admin", "manager", "hr", "employee"] },
  { key: "quotation", labelEn: "Quotations", labelAr: "العروض", descEn: "Quotation submissions and approvals", descAr: "تقديم العروض والموافقات", roles: ["admin", "manager", "finance"] },
];
...
export function savePrefs(p: NotifPrefs, role?: Role) {
  ...
  try { localStorage.setItem(keyFor(role), JSON.stringify(p)); } catch { /* ignore */ }
...
export function useNotifPrefs(role?: Role): [NotifPrefs, (p: NotifPrefs) => void] {
  ...
  return [prefs, (p) => { savePrefs(p, role); setPrefs(p); }];
}

// Map a notification to its category for filtering by user prefs.
export function categoryOf(n: Pick<AppNotification, "type" | "titleEn" | "bodyEn" | "href">): NotifCategory {
...
export function isAllowed(n: Pick<AppNotification, "type" | "titleEn" | "bodyEn" | "href">, prefs: NotifPrefs): boolean {
