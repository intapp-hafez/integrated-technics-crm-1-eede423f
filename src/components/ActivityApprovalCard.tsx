...
export function ActivityApprovalCard({
  ...
  useEffect(() => {
    setNote(activity.reviewNote ?? "");
  }, [activity.id, activity.reviewNote]);
  ...
  useEffect(() => {
    loadAttachments(); /* eslint-disable-next-line */
  }, [activity.id]);

  const handleUpload = async (file: File) => {
    if (!isUuid(activity.id)) {
      toast.error("Cannot attach: activity not saved yet");
      return;
    }
    setUploading(true);
    try {
  ...
  return (
    <div
      className="mt-2 w-full rounded-xl border border-border bg-secondary/30 p-3"
    ...
    >
      ...
      {status !== "pending" && (
        <div
          className={`mt-3 rounded-lg border p-2.5 text-xs ${status === "approved" ? "border-emerald-200 bg-emerald-50/60" : "border-rose-200 bg-rose-50/60"}`}
        >
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span
              className={`font-bold uppercase tracking-wider text-[10px] ${status === "approved" ? "text-emerald-700" : "text-rose-700"}`}
            >
          ...
          {activity.rejectionReason && (
            <div className="mt-1 text-rose-700">
              <strong>Reason:</strong> {activity.rejectionReason}
            </div>
          )}
          {activity.reviewNote && (
            <div className="mt-1 text-foreground">
              <strong>Note:</strong> {activity.reviewNote}
            </div>
          )}
        </div>
      )}
      ...
      <div className="mt-3">
        <div className="mb-1.5 flex items-center justify-between">
          ...
          <label
            className={`inline-flex cursor-pointer items-center gap-1 rounded-md bg-primary/10 px-2 py-1 text-[11px] font-semibold text-primary ring-1 ring-primary/20 hover:bg-primary/20 ${uploading ? "opacity-60 pointer-events-none" : ""}`}
          >
        ...
        {attachments.length > 0 && (
          <ul className="space-y-1">
            {attachments.map((att) => (
              <li
                key={att.id}
                className="flex items-center gap-2 rounded-md bg-card px-2 py-1.5 ring-1 ring-border"
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[10px] text-muted-foreground">
                    {fmtSize(att.size_bytes)} · {att.mime ?? "file"}
                  </div>
                </div>
                <button
                  onClick={() => openOrDownload(att, "preview")}
                ...
                <button
                  onClick={() => openOrDownload(att, "download")}
                  className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-[11px] font-semibold text-foreground hover:bg-accent"
                  title="Download"
                >
      ...
      {canApprove && status === "pending" && (
        <div className="mt-3 space-y-2">
          <label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
            Review note (optional, shared with creator)
          </label>
          <textarea
            value={note}
