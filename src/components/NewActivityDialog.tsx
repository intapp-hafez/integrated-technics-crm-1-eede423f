    ...
    >
      ...
      >
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary">
                {dir === "rtl" ? "نشاط جديد" : "New Activity"}
              </div>
              <h3 className="font-display text-lg font-bold text-foreground">
                {dir === "rtl"
                  ? "ابدأ بالحساب، ثم العميل، ثم الموظف"
                  : "Start with the account, then the lead, then the assignee"}
              </h3>
            </div>
            ...
            {stepMeta.map((s, i) => {
              ...
              return (
                <div key={s.n} className="flex items-center gap-2">
                  ...
                  {i < stepMeta.length - 1 && (
                    <div className={`h-px w-6 ${done ? "bg-emerald-300" : "bg-border"}`} />
                  )}
                </div>
              );
            })}
        ...
        <div className="px-6 py-5">
            ...
            <div>
              <div className="mb-3 rounded-lg bg-secondary/60 p-3 text-xs">
                ...
                <div className="text-muted-foreground">
                  {dir === "rtl" ? "العميل:" : "Client:"} {project.client}
                </div>
              </div>
              <div className="mb-3 text-xs font-semibold text-muted-foreground">
                {dir === "rtl"
                  ? "اختر العميل المحتمل المرتبط"
                  : "Pick a related lead (or skip for a general project activity)"}
              </div>
              {projectLeads.length === 0 ? (
                <div className="rounded-lg border border-dashed border-border p-4 text-center text-xs text-muted-foreground">
                  {dir === "rtl"
                    ? "لا توجد عملاء محتملون مرتبطون بهذا المشروع"
                    : "No leads found for this project's client."}
                </div>
              ) : (
                <div className="space-y-2 max-h-[280px] overflow-y-auto pr-1">
                  ...
                  >
                    <div className="text-sm font-semibold text-foreground">
                      — {dir === "rtl" ? "بدون عميل محدد" : "No specific lead"} —
                    </div>
                    <div className="text-[11px] text-muted-foreground">
                      {dir === "rtl"
                        ? "نشاط عام على مستوى المشروع"
                        : "General project-level activity"}
                    </div>
                  </button>
                  {projectLeads.map((l) => {
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <div className="text-[11px] text-muted-foreground">
                              {l.industry} · {l.city} · {shortId(l.id)}
                            </div>
                          </div>
              ...
              <div className="flex items-center gap-3 rounded-xl border border-primary/40 bg-primary/5 p-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate text-sm font-bold text-foreground">
                    {myName || (dir === "rtl" ? "أنت" : "You")}
                  </div>
                  <div className="truncate text-[11px] text-muted-foreground">
                    {profile?.title || (dir === "rtl" ? "موظف" : "Employee")}
                  </div>
                </div>
          ...
          {step === 3 && canAssignOthers && (
            <div>
              <div className="mb-3 text-xs font-semibold text-muted-foreground">
                {dir === "rtl"
                  ? "الموظفون المقترحون لهذا الحساب/العميل"
                  : "Suggested assignees based on this account & lead"}
              </div>
              {suggestedOwners.length > 0 && (
                <div className="mb-4">
                  <div className="mb-2 text-[10px] font-bold uppercase tracking-wider text-primary">
                    {dir === "rtl" ? "مقترحون" : "Suggested"}
                  </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="truncate text-[11px] text-muted-foreground">
                              {emp?.role ?? (dir === "rtl" ? "مالك العميل" : "Lead owner")}
                            </div>
                          </div>
              ...
              <div>
                <div className="mb-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                  {dir === "rtl" ? "كل الموظفين" : "All employees"}
                </div>
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-xl border border-border bg-secondary/40 p-3 text-xs">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-card px-2.5 py-1 font-semibold text-foreground ring-1 ring-border">
                    <Briefcase className="h-3 w-3 text-primary" /> {project?.name}
                  </span>
                  {lead && (
                    <span className="inline-flex items-center gap-1.5 rounded-full bg-card px-2.5 py-1 font-semibold text-foreground ring-1 ring-border">
                      <Building2 className="h-3 w-3 text-primary" /> {lead.contact}
                    </span>
                  )}
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-card px-2.5 py-1 font-semibold text-foreground ring-1 ring-border">
                    <UserCircle2 className="h-3 w-3 text-primary" /> {owner}
                  </span>
                </div>
              </div>
              ...
              <Field label={dir === "rtl" ? "العنوان" : "Title"} icon={FileText}>
                <input
                  ...
                  placeholder={
                    dir === "rtl"
                      ? "مثال: مكالمة استكشافية — Aramco"
                      : "e.g. Discovery call — Aramco"
                  }
                />
              </Field>
              ...
              <Field
                label={dir === "rtl" ? "الوقت المقدّر (دقيقة)" : "Estimated time (minutes)"}
                icon={Timer}
              >
                <input
                  type="number"
        ...
        <div className="flex items-center justify-between gap-2 border-t border-border bg-secondary/30 px-6 py-3">
          <button
            onClick={onClose}
            className="rounded-lg px-3 py-2 text-sm font-semibold text-muted-foreground hover:bg-accent"
          >
