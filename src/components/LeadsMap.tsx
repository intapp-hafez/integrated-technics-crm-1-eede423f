...
function aggregateOwners(
  leads: Lead[],
  lookup: (name: string) => EmpMini | undefined,
): OwnerInfo[] {
  ...
  for (const l of leads) {
    ...
    if (cur) {
      ...
      m.set(l.owner, {
        name: l.owner,
        photo: (l as any).ownerPhoto ?? emp?.photo,
        initials: initialsOf(l.owner),
        count: 1,
      });
    }
  }
...
export function LeadsMap({ leads }: { leads: Lead[] }) {
  ...
  const empLookup = useMemo(() => {
    ...
    for (const e of storeEmployees as any[])
      if (e?.name) m.set(e.name, { name: e.name, photo: e.photo || m.get(e.name)?.photo });
    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_320px]">
                ...
                <Popup>
                  <div style={{ minWidth: 200 }}>
                    ...
                    <div style={{ fontSize: 12, color: "#666" }}>
                      {c.count} leads · {fmtMoney(c.value)}
                    </div>
                    <div style={{ marginTop: 6, fontSize: 11, color: "#444", fontWeight: 600 }}>
                      Owners
                    </div>
                          ...
                          {o.photo ? (
                            ...
                            <span
                              style={{
                                ...
                                height: 20,
                                borderRadius: 9999,
                                background: "oklch(0.706 0.181 49.5)",
                                color: "#fff",
                                fontSize: 9,
                                fontWeight: 700,
                              }}
      ...
      <div className="rounded-xl border border-border bg-card p-4 shadow-[var(--shadow-soft)]">
        ...
        <div className="mt-2 rounded-lg bg-secondary/50 p-2 text-xs text-muted-foreground">
          {selected ? (
            <>
              <span className="font-semibold text-foreground">{selected.city}</span> ·{" "}
              {selected.count} leads · {fmtMoney(selected.value)}
            </>
          ) : (
            <>
