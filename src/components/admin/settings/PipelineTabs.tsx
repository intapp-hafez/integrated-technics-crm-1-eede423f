  ...
  return (
    <div>
        ...
        {settings.statuses.map((s) => {
          ...
          return (
            ...
            >
              ...
              {label}
              {!locked && (
                <button onClick={() => actions.removeStatus(s)} className="ml-1 rounded p-0.5 text-current/70 hover:bg-black/10" aria-label={`Remove ${label}`}>
                  <X className="h-3 w-3" />
                </button>
              )}
            </span>
      ...
      <p className="mt-3 text-xs text-muted-foreground">
        Statuses sync to the pipeline, lead forms, and filters. New, Won, and Lost are protected and can't be removed.
      </p>
    </div>
  );
}
...
function StageRow({ stageKey, label, color, index, total }: { stageKey: string; label: string; color: string; index: number; total: number }) {
  ...
  return (
    <div
      draggable
      onDragStart={(e) => { e.dataTransfer.effectAllowed = "move"; e.dataTransfer.setData("text/stage-index", String(index)); }}
      onDragOver={(e) => { e.preventDefault(); const rect = (e.currentTarget as HTMLDivElement).getBoundingClientRect(); setDragOver(e.clientY < rect.top + rect.height / 2 ? "top" : "bottom"); }}
    ...
    >
      ...
      <span className="flex h-7 w-5 cursor-grab items-center justify-center text-muted-foreground hover:text-foreground active:cursor-grabbing" aria-label="Drag to reorder">
        <GripVertical className="h-4 w-4" />
      </span>
      <span className="w-6 text-center font-mono text-[11px] text-muted-foreground">{index + 1}</span>
      <input type="color" value={color} onChange={(e) => actions.setStageColor(stageKey, e.target.value)} className="h-7 w-7 cursor-pointer rounded border border-border bg-transparent p-0" aria-label="Stage color" />
      <span className="font-mono text-[11px] uppercase tracking-wider text-muted-foreground">{t(stageKey as any) ?? stageKey}</span>
      <input value={val} onChange={(e) => setVal(e.target.value)} className="h-9 flex-1 rounded-md border border-border bg-card px-2 text-sm focus:border-primary focus:outline-none" />
      <button
        disabled={!dirty}
      ...
      {!locked && (
        <button onClick={() => actions.removeStatus(stageKey)} className="inline-flex h-8 w-8 items-center justify-center rounded-md text-muted-foreground hover:bg-rose-50 hover:text-rose-600" aria-label="Delete stage">
      ...
      <div className="flex flex-wrap gap-2">
        {settings.activityTypes.map((tp) => (
          <span key={tp} className="inline-flex items-center gap-1.5 rounded-full bg-secondary px-3 py-1.5 text-sm font-semibold text-foreground ring-1 ring-border">
            {tp}
            <button onClick={() => actions.removeActivityType(tp)} className="ml-0.5 inline-flex h-4 w-4 items-center justify-center rounded-full text-muted-foreground hover:bg-destructive/10 hover:text-destructive" aria-label={`Remove ${tp}`}>
              <X className="h-3 w-3" />
            </button>
          </span>
        ))}
