...
function weekBounds(now = new Date()) {
  ...
  const monday = new Date(
    Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate() - diffToMon, 0, 0, 0),
  );
...
function countMeetingsBetween(
  activities: EmployeeTargetsCardProps["activities"],
  startIso: string,
  endIso: string,
) {
...
export function EmployeeTargetsCard({
  userId,
  profileId,
  leads,
  activities,
  ...
  const profileQuery = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from("profiles")
        .select(
          "id,start_date,annual_target,q1_target,q2_target,q3_target,q4_target,weekly_meetings_target",
        );
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes" as any,
        {
          ...
          schema: "public",
          table: "profiles",
          filter: userId ? `user_id=eq.${userId}` : `id=eq.${profileId}`,
        },
        () => {
  ...
  const annualAchieved = useMemo(
    () => sumWonBetween(leads, yearStart, yearEnd),
    [leads, yearStart, yearEnd],
  );
  const currentQ = (Math.floor(m / 3) + 1) as 1 | 2 | 3 | 4;
  const qAchieved = useMemo(
    () =>
      [1, 2, 3, 4].map((qn) => {
  ...
  const tenureDays = startDate
    ? Math.max(
        0,
        Math.floor((Date.now() - new Date(startDate + "T00:00:00Z").getTime()) / 86400_000),
      )
    : null;

  const meetingsPct = meetingsTarget > 0 ? Math.min(100, (meetingsDone / meetingsTarget) * 100) : 0;
  const meetingsTone: Parameters<typeof Bar>[0]["tone"] =
    meetingsPct >= 100 ? "emerald" : meetingsPct >= 60 ? "amber" : "rose";
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-soft)]">
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div
            className={`text-[11px] font-bold uppercase ${isAr ? "" : "tracking-widest"} text-primary mb-1`}
          >
            {L.perfMetrics}
          </div>
        ...
        <>
          ...
          <div className="mb-5 flex flex-wrap items-center gap-3">
            <div className="flex flex-wrap items-center gap-2 rounded-xl bg-secondary/40 px-4 py-2 border border-border">
              ...
              <span
                className={`text-[10px] font-bold uppercase ${isAr ? "" : "tracking-wider"} text-muted-foreground`}
              >
                {L.employmentStart}
              </span>
              ...
              {tenureDays !== null && (
                <span className="text-xs text-muted-foreground">
                  · {tenureDays} {L.days} · {(tenureDays / 365).toFixed(1)} {L.yrs}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
            ...
            <div className="md:col-span-2 relative overflow-hidden rounded-2xl bg-slate-900 p-6 text-white shadow-lg">
              <div className="relative z-10 flex flex-col h-full justify-between">
                <div>
                  <p
                    className={`text-slate-400 text-xs font-semibold uppercase ${isAr ? "" : "tracking-wider"} mb-2`}
                  >
                    {L.annualRevenueTarget} ({y})
                  </p>
                  <div className="flex items-baseline gap-3 flex-wrap">
                    <h3 className="text-3xl sm:text-4xl font-black tracking-tight" dir="ltr">
                      {fmtMoney(annualTarget)}
                    </h3>
                    {annualTarget > 0 && (
                      <span className="bg-primary/20 text-primary text-[10px] font-bold px-2 py-0.5 rounded-full ring-1 ring-primary/30">
                        {annualPct.toFixed(0)}% {L.achieved}
                      </span>
                    )}
                  </div>
                </div>
                <div className="mt-6">
                  <div className="flex justify-between gap-3 text-xs font-bold mb-2">
                    <span className="text-slate-400">
                      {L.progress}: {annualPct.toFixed(0)}%
                    </span>
                    <span className="text-primary" dir={isAr ? "rtl" : "ltr"}>
                      {fmtMoney(annualAchieved)} {L.achieved}
                    </span>
                  </div>
              ...
              <div
                className={`absolute ${isAr ? "-left-12" : "-right-12"} -top-12 w-48 h-48 bg-primary/10 rounded-full blur-3xl`}
              />
            </div>
            ...
            <div className="rounded-2xl border-2 border-secondary/60 bg-card p-5 flex flex-col justify-between shadow-sm">
              ...
              <div className="mt-5">
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className="text-[10px] font-black text-muted-foreground w-8 text-end">
                    {meetingsPct.toFixed(0)}%
                  </span>
                </div>
                <div
                  className={`text-[11px] font-bold ${meetingsPct >= 100 ? "text-emerald-600" : meetingsPct >= 60 ? "text-amber-600" : "text-rose-600"}`}
                >
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-center mb-4 gap-2">
                    <span
                      className={`text-[10px] font-black uppercase ${isAr ? "" : "tracking-widest"} transition-colors ${
                        isFuture
                          ? "text-muted-foreground"
                          : "text-muted-foreground group-hover:text-primary"
                      }`}
                    >
                      {isAr ? `${L.focus} الربع ${qn}` : `Q${qn} ${L.focus}`}
                    </span>
                    <div
                  ...
                  <p
                    className={`text-xl font-extrabold ${isFuture ? "text-muted-foreground" : "text-foreground"}`}
                    dir="ltr"
                  >
                    {fmtMoney(t)}
                  </p>
                  <p
                    className={`text-[10px] font-medium mb-3 ${isFuture ? "text-muted-foreground/60" : "text-muted-foreground"}`}
                  >
                    {isPast
                      ? L.completed
                      : isCurrent
                        ? `${pct.toFixed(0)}% ${L.progressShort}`
                        : L.scheduled}
                  </p>
                  ...
                  {!isFuture && (
                    <p
                      className="mt-2 text-[10px] font-bold text-muted-foreground"
                      dir={isAr ? "rtl" : "ltr"}
                    >
      ...
      {!profileQuery.isLoading && (
        <div className="mt-6 rounded-lg border border-border bg-background p-4">
          <div className="mb-3 flex items-center justify-between gap-3 flex-wrap">
            ...
            <div
              className={`text-[10px] uppercase ${isAr ? "" : "tracking-wider"} text-muted-foreground`}
            >
              {L.actualVsTarget}
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[640px] text-sm">
              <thead>
                <tr
                  className={`border-b border-border text-[10px] uppercase ${isAr ? "" : "tracking-wider"} text-muted-foreground`}
                >
              ...
              <tbody>
                {qTargets.map((tg, i) => {
                  ...
                  const tone: Parameters<typeof Bar>[0]["tone"] =
                    pct >= 100
                      ? "emerald"
                      : pct >= 60
                        ? "amber"
                  ...
                  return (
                    <tr
                      key={qn}
                      className={`border-b border-border/60 last:border-0 ${isCurrent ? "bg-primary/5" : ""}`}
                    >
                      <td className="py-2.5 font-bold text-foreground">
                        Q{qn}
                        {isCurrent && (
                          <span
                            className={`${isAr ? "mr-1" : "ml-1"} rounded-sm bg-primary/15 px-1 py-0.5 text-[9px] uppercase tracking-wider text-primary`}
                          >
                            {L.now}
                          </span>
                        )}
                      ...
                      <td className="py-2.5 text-end font-mono text-xs text-foreground" dir="ltr">
                        {fmtMoney(tg)}
                      </td>
                      <td
                        className="py-2.5 text-end font-mono text-xs font-bold text-foreground"
                        dir="ltr"
                      >
                        {fmtMoney(ach)}
                      </td>
                      <td
                        className={`py-2.5 text-end font-mono text-xs font-semibold ${gap >= 0 ? "text-emerald-600" : "text-rose-600"}`}
                        dir="ltr"
                      >
                        {gap >= 0 ? "+" : ""}
                        {fmtMoney(gap)}
                      </td>
                      <td className="py-2.5 text-end text-xs text-foreground" dir="ltr">
                        {qWonCounts[i]}
                      </td>
                      <td className="py-2.5 text-end text-xs text-foreground" dir="ltr">
                        {qMeetings[i]}
                      ...
                      <td className="py-2.5 px-2">
                        <div className="flex items-center gap-2">
                          <div className="flex-1">
                            <Bar value={ach} max={tg} tone={tone} />
                          </div>
                          <span
                            className={`w-10 text-end text-[11px] font-bold ${pct >= 100 ? "text-emerald-600" : pct >= 60 ? "text-amber-600" : "text-rose-600"}`}
                            dir="ltr"
                          >
                            {pct.toFixed(0)}%
                          </span>
                ...
                {(() => {
                  ...
                  const pct = tg > 0 ? Math.min(100, (ach / tg) * 100) : 0;
                  const tone: Parameters<typeof Bar>[0]["tone"] =
                    pct >= 100 ? "emerald" : pct >= 60 ? "amber" : "primary";
                  return (
                    <tr className="bg-secondary/40">
                      <td className="py-2.5 font-bold text-foreground" colSpan={2}>
                        {L.yearTotal}
                      </td>
                      <td
                        className="py-2.5 text-end font-mono text-xs font-bold text-foreground"
                      ...
                      <td
                        className={`py-2.5 text-end font-mono text-xs font-bold ${gap >= 0 ? "text-emerald-600" : "text-rose-600"}`}
                        dir="ltr"
                      >
                        {gap >= 0 ? "+" : ""}
                        {fmtMoney(gap)}
                      </td>
                      <td className="py-2.5 text-end text-xs font-bold text-foreground" dir="ltr">
                        {qWonCounts.reduce((s, v) => s + v, 0)}
                      </td>
                      <td className="py-2.5 text-end text-xs font-bold text-foreground" dir="ltr">
                        {qMeetings.reduce((s, v) => s + v, 0)}
                      </td>
                      <td className="py-2.5 px-2">
                        <div className="flex items-center gap-2">
                          <div className="flex-1">
...
const inputCls =
  "h-9 w-full rounded-lg border border-border bg-background px-3 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary";
