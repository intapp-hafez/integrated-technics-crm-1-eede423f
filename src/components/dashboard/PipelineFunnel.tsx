...
export function PipelineFunnel({ leads }: Props) {
  ...
  const getStageValue = (key: string) => activeLeads.filter(l => l.status === key).reduce((sum, l) => sum + (l.value || 0), 0);
          ...
          return (
            <div key={stage.key} className="relative group">
              <div className="flex items-end justify-between mb-1 text-sm">
                ...
                <span className="text-muted-foreground font-mono text-xs">{count} leads ({fmtMoney(value)})</span>
              </div>
