...
function isPrivateHost(host: string): boolean {
  ...
  if (h === "localhost" || h.endsWith(".local") || h.endsWith(".lan") || h.endsWith(".intranet"))
    return true;
...
const REQUIRED_HEADERS: HeaderCheck[] = [
  {
    ...
    required: true,
    description: "Forces HTTPS for return visits.",
  },
  {
    name: "Content-Security-Policy",
    required: true,
    description: "Mitigates XSS by limiting allowed sources.",
  },
  ...
  { name: "X-Content-Type-Options", required: true, description: "Disables MIME sniffing." },
  {
    ...
    required: true,
    description: "Prevents URL leakage to third parties.",
  },
...
function StatusPill({ status, children }: { status: CheckStatus; children: React.ReactNode }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-bold uppercase tracking-wider ring-1 ${cls}`}
    >
      {children}
    </span>
  );
    ...
    <div className="overflow-hidden rounded-xl border border-border bg-slate-950">
      <div className="flex items-center justify-between border-b border-slate-800 bg-slate-900 px-3 py-1.5">
        ...
        <div className="flex gap-1">
          <button
            onClick={copy}
            className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-[11px] font-semibold text-slate-200 hover:bg-slate-800"
          >
      ...
      <pre className="max-h-[320px] overflow-auto px-3 py-3 text-[11px] leading-relaxed text-slate-100">
        <code>{code}</code>
      </pre>
    </div>
  );
...
function SecurityCenterPage() {
  ...
  useEffect(() => {
    ...
    const onShow = () => {
      if (!blackBox) document.body.classList.remove("sec-blackbox");
    };
      ...
      >
        <div className="mx-auto max-w-md rounded-2xl border border-border bg-card p-10 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            You need admin privileges to view the Security Center.
          </p>
        </div>
      </AppShell>
    );
        ...
        <div className="flex flex-wrap items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm text-muted-foreground">
              Hardening guidance for an IIS deployment restricted to private IP ranges. Use the live
              scanner and copy the IIS snippets into your{" "}
              <span className="font-mono">web.config</span>.
            </p>
          </div>
          {hostInfo && (
      ...
      {activeTab === "config" && (
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
          ...
          <Section
            ...
            subtitle="Verify response headers and apply the IIS template."
          >
            <div className="mb-3 flex items-center justify-between">
              ...
              >
                ...
                {headersLoading ? "Scanning…" : "Scan current origin"}
              </button>
              {headerResults && (
                <span className="text-[11px] text-muted-foreground">
                  {headerResults.filter((r) => r.status === "pass").length}/{headerResults.length}{" "}
                  present
                </span>
              )}
            </div>
            ...
            <p className="mt-3 text-[11px] text-muted-foreground">
              ...
              Some headers are stripped from preview proxies — re-run this scan against your IIS
              host for accurate results.
            </p>
          </Section>

          {/* Black-boxing */}
          <Section
            icon={blackBox ? EyeOff : Eye}
            title="Black-Boxing (Screen Privacy)"
            subtitle="Mask sensitive cells when sharing a screen."
          >
            <div className="rounded-lg border border-border p-3">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm font-semibold text-foreground">
                    Mask values across the app
                  </div>
                ...
                >
                  <span
                    className={`inline-block h-5 w-5 transform rounded-full bg-white shadow transition ${blackBox ? "translate-x-5" : "translate-x-0.5"}`}
                  />
                </button>
              </div>
              <div className="mt-3 rounded-md border border-dashed border-border p-3 text-xs">
                ...
                <div className="mt-1 flex flex-wrap gap-3">
                  <span>
                    Email:{" "}
                    <span className="sec-mask" data-sensitive>
              ...
              <div className="mt-3 flex items-center justify-between rounded-md border border-border bg-muted/30 p-2">
                <div className="min-w-0">
                  <div className="text-xs font-semibold text-foreground">
                    Auto-mask when tab loses focus
                  </div>
                  <p className="text-[11px] text-muted-foreground">
                    Hides sensitive values on Alt-Tab, screen-share switch, or window blur.
                  </p>
                </div>
                <button
                ...
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition ${autoMaskOnBlur ? "translate-x-4" : "translate-x-0.5"}`}
                  />
                </button>
              </div>

              <p className="mt-2 text-[11px] text-muted-foreground">
                Add the <span className="font-mono">sec-mask</span> class or{" "}
                <span className="font-mono">data-sensitive</span> attribute to any element that
                should hide during demos and shoulder-surfing.
              </p>
            </div>
          </Section>
          ...
          <Section
            ...
            subtitle="Active client-side defenses against framing, copy theft and console attacks."
          >
            <div className="rounded-lg border border-border p-3">
              <div className="flex items-center justify-between">
                <div className="min-w-0">
                  <div className="text-sm font-semibold text-foreground">
                    Enable defensive runtime
                  </div>
                  <p className="text-[11px] text-muted-foreground">
                    Frame-busts foreign iframes, blocks right-click & copy on masked fields,
                    intercepts Ctrl/Cmd+P/S, auto-masks on PrintScreen and devtools open, and prints
                    a self-XSS warning in the console.
                  </p>
                </div>
                <button
                ...
                >
                  <span
                    className={`inline-block h-5 w-5 transform rounded-full bg-white shadow transition ${antiTamper ? "translate-x-5" : "translate-x-0.5"}`}
                  />
                </button>
              </div>
              ...
              <p className="mt-2 text-[11px] text-amber-700">
                ...
                Client-side hardening only — keep CSP, HSTS and IP allowlists on IIS as the primary
                defense.
              </p>
            </div>
          </Section>
          ...
          <Section
            icon={Bug}
            title="XSS Defense"
            subtitle="What we enforce in code + what the server must enforce."
          >
            <ul className="space-y-2 text-xs">
              <li className="flex items-start gap-2">
                <StatusPill status="pass">In code</StatusPill>
                <span>
                  React escapes interpolated text by default — never call{" "}
                  <span className="font-mono">dangerouslySetInnerHTML</span> with user input.
                </span>
              </li>
              <li className="flex items-start gap-2">
                <StatusPill status="pass">In code</StatusPill>
                <span>
                  All form inputs validated with <span className="font-mono">zod</span> schemas
              ...
              <li className="flex items-start gap-2">
                ...
                <span>
                  Apply the <span className="font-mono">Content-Security-Policy</span> header from
                  the IIS template below to block injected scripts at the browser.
                </span>
              </li>
              <li className="flex items-start gap-2">
                <StatusPill status="warn">Server</StatusPill>
                <span>
                  Keep <span className="font-mono">X-Content-Type-Options: nosniff</span> so a
                  tampered upload can't execute as a script.
                </span>
              </li>
              <li className="flex items-start gap-2">
                <StatusPill status="info">Audit</StatusPill>
                <span>
                  Search the repo for <span className="font-mono">dangerouslySetInnerHTML</span>,{" "}
                  <span className="font-mono">eval(</span> and{" "}
                  <span className="font-mono">new Function(</span> before each release.
                </span>
              </li>
            </ul>
          </Section>
          ...
          <Section
            icon={Network}
            title="Networking · Private IPs"
            subtitle="Restrict the IIS site to RFC1918 ranges only."
          >
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <ul className="mt-1 list-disc pl-4 text-muted-foreground">
                    ...
                    <li>
                      <span className="font-mono">127.0.0.1</span> (loopback)
                    </li>
                  </ul>
                </div>
              </div>
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">
                    Install the <span className="font-mono">IP and Domain Restrictions</span> and{" "}
                    <span className="font-mono">URL Rewrite</span> role services. Bind the site to
                    the internal NIC only; do not publish port 80/443 on the public firewall.
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">
                    Issue an internal CA certificate (or use AD Certificate Services) and bind HTTPS
                    — HSTS only works over TLS.
                  </p>
                </div>
              </div>
            </div>
      ...
      {activeTab === "config" && (
        <div className="mt-6 space-y-5">
          <h3 className="font-display text-lg font-bold text-foreground">
            IIS <span className="font-mono text-base">web.config</span> templates
          </h3>
          ...
          <p className="text-[11px] text-muted-foreground">
            Merge these snippets into a single <span className="font-mono">web.config</span> at the
            IIS site root. After deploying, re-run "Scan current origin" above to confirm the
            headers are live.
          </p>
        </div>
      )}
