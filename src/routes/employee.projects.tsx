...
function ProjectsPage() {
  ...
  const approvedRequests = (projectRequests || []).filter((req: any) => req.status === "approved" && req.requested_by === profile.profileId);
      ...
      <div className="mb-6 flex gap-2 border-b border-border">
        ...
        >
          ...
          <span className={`rounded-full px-2 py-0.5 text-[10px] ${
            activeTab === "accounts" ? "bg-primary/10 text-primary" : "bg-secondary text-muted-foreground"
          }`}>
            {myProjects.length}
          </span>
        </button>
