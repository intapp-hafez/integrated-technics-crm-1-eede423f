...
function ManagerLeadsListPage() {
  ...
  useEffect(() => { setPage(1); }, [q, status, owner]);
    ...
    <AppShell panel="manager" user={user} pageTitle={isAr ? "فرص فريقنا" : "Our Leads"}>
      ...
      <div className="mb-4 flex flex-col gap-3">
        <div className="flex items-center gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          <button onClick={() => setEditing("new")} className="shrink-0 inline-flex h-9 items-center gap-2 rounded-lg bg-primary px-3 text-xs font-semibold text-primary-foreground shadow-[var(--shadow-brand)] active:scale-[0.98] hover:bg-primary/90">
                ...
                <tr key={l.id} className="transition hover:bg-primary/5">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => setEditing(l)} className="text-muted-foreground hover:text-primary ms-auto" aria-label="Edit">
      ...
      {editing && (
        <LeadFormModal initial={editing === "new" ? null : editing} locations={settings.locations} teamEmployees={teamEmployees} user={user} onClose={() => setEditing(null)} />
      )}
      {showImport && (
...
function LeadFormModal({ initial, locations, teamEmployees, user, onClose }: { initial: Lead | null; locations: LocationCity[]; teamEmployees: any[]; user: any; onClose: () => void }) {
  const { t, lang } = useI18n();
  const isAr = lang === "ar";
        ...
        <div className="grid max-h-[70vh] grid-cols-1 gap-3 overflow-y-auto sm:grid-cols-2">
          ...
          <Field label="Assign to">
            <select value={owner} onChange={(e) => setOwner(e.target.value)} className="h-9 w-full rounded-lg border border-border bg-background px-2 text-sm">
              ...
              {teamEmployees.filter(emp => emp.name !== user.name).map((e) => (
