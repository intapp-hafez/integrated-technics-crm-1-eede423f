      ...
      <div className="mx-auto w-full max-w-md md:max-w-6xl">
        <div className="rounded-2xl border border-border bg-card p-4 shadow-[var(--shadow-soft)]">
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-sm font-bold text-foreground">
              {fmtH(doneTodayMins)}{" "}
              <span className="text-muted-foreground">/ {fmtH(todayMins)}</span>
            </span>
          </div>
        </div>
        ...
        <div className="mt-5 flex items-center gap-1.5 overflow-x-auto pb-1">
          <div className="flex items-center gap-1 rounded-full bg-card p-0.5 ring-1 ring-border">
            ...
            <button
              ...
              className={`flex h-7 w-7 items-center justify-center rounded-full ${view === "calendar" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
            >
        ...
        {view === "calendar" && (
          <CalendarView
            activities={mine}
            onPick={(id) =>
              navigate({ to: "/employee/activities/$activityId", params: { activityId: id } })
            }
          />
        )}
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <div
                        className={`truncate text-sm font-bold ${done ? "text-foreground" : "text-foreground"}`}
                      >
                        {a.title}
                      </div>
                      ...
                      {a.approvalStatus === "rejected" && (
                        <span
                          className="rounded-full bg-rose-100 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-rose-700"
                    ...
                    <div className="flex items-center gap-2">
                      <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                        {a.dueDate === today ? t("today").toUpperCase() : a.dueDate} · {a.time}
...
function EditActivityCard({
  ...
  activity: ReturnType<typeof useStoreState>["activities"][number];
  onClose: () => void;
}) {
  const { t } = useI18n();
...
function CalendarView({
  ...
  activities: ReturnType<typeof useStoreState>["activities"];
    ...
    <div className="mt-4 rounded-2xl border border-border bg-card p-4 shadow-[var(--shadow-soft)]">
        ...
        {cells.map((c, i) => {
          ...
          return (
            ...
            >
              ...
              {count > 0 && (
                <span
                  className={`absolute bottom-1 left-1/2 -translate-x-1/2 flex h-4 w-4 items-center justify-center rounded-full text-[10px] font-bold ${isSelected ? "bg-primary-foreground text-primary" : "bg-emerald-100 text-emerald-700"}`}
                >
                  {count}
                </span>
              )}
      ...
      <div className="mt-4 flex items-center justify-between border-t border-border pt-3">
        ...
        <div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
          {dayActivities.length} item(s) · {fmtH(totalSelected)}
        </div>
      </div>
        ...
        {dayActivities.map((a) => {
          ...
          return (
            <button
              key={a.id}
              onClick={() => onPick(a.id)}
              className="flex w-full items-center gap-2.5 rounded-xl bg-background p-2.5 text-start ring-1 ring-border hover:border-primary/40 hover:ring-primary/40"
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[10px] text-muted-foreground">
                  {a.time}
                  {a.estMinutes ? ` · ${fmtH(a.estMinutes)}` : ""} · {a.status.replace("_", " ")}
                </div>
              </div>
            </button>
          );
      ...
      <div className="mt-2 grid grid-cols-3 gap-2">
        ...
        <div className="flex items-center gap-1 rounded-lg border border-border bg-background px-2 text-xs">
          ...
          <input
            ...
            value={mins}
            onChange={(e) => setMins(Number(e.target.value))}
            className="h-9 w-full bg-transparent text-xs focus:outline-none"
          />
