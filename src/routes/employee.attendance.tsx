...
function AttendancePage() {
  ...
  const handleCheckIn = async () => {
    ...
    actions.addAttendance({
      date: today, checkIn: now, checkOut: "", hours: "—",
      location: locName, owner: profile.name,
  ...
  const handleCheckOut = async () => {
    ...
    const now = cairoTime();
    const pos = await getCurrentPosition();
    const patch: Record<string, any> = { checkOut: now, hours: computeHours(todayRec.checkIn, now) };
    if (pos) {
    ...
    <AppShell panel="employee" user={{ name: profile.name, role: t("employee"), initials: profile.name.split(" ").map((s) => s[0]).join("").slice(0, 2) }} pageTitle={t("attendance")}>
      <div className="rounded-2xl border border-border p-6 shadow-[var(--shadow-soft)]" style={{ background: "var(--gradient-brand)" }}>
        <div className="flex flex-col items-center gap-4 text-center text-white">
          ...
          <div>
            <div className="text-xs uppercase tracking-widest text-white/70">{t("today")} · {todayRec?.location || profile.location}</div>
            <div className="font-display text-4xl font-bold">{todayRec ? todayRec.checkIn : "—:—"}</div>
            <div className="mt-1 text-sm text-white/80">{t("workingHours")}: {todayRec ? (todayRec.hours !== "—" ? todayRec.hours : computeHours(todayRec.checkIn, cairoTime())) : "—"}</div>
          </div>
          <div className="flex flex-wrap justify-center gap-3">
            ...
            {!todayRec ? (
              <button disabled={locating} onClick={handleCheckIn} className="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-semibold text-foreground hover:bg-white/90 disabled:opacity-60">
                {locating ? <Loader2 className="h-4 w-4 animate-spin" /> : <LogIn className="h-4 w-4" />} {t("checkIn")}
              </button>
            ) : !todayRec.checkOut ? (
              <button disabled={locating} onClick={handleCheckOut} className="inline-flex items-center gap-2 rounded-lg bg-foreground px-4 py-2 text-sm font-semibold text-background hover:bg-foreground/90 disabled:opacity-60">
                {locating ? <Loader2 className="h-4 w-4 animate-spin" /> : <LogOut className="h-4 w-4" />} {t("checkOut")}
              </button>
            ) : (
      ...
      {mapCenter && (
        <div className="mt-6 overflow-hidden rounded-xl border border-border bg-card shadow-[var(--shadow-soft)]">
          <div className="flex items-center gap-2 border-b border-border px-5 py-3">
            ...
            <span className="ms-auto font-mono text-xs text-muted-foreground">{mapCenter[0].toFixed(5)}, {mapCenter[1].toFixed(5)}</span>
          </div>
...
type AttRecord = { id: string; date: string; checkIn: string; checkOut: string; hours: string; location: string; lat?: number | null; lng?: number | null; accuracy?: number | null };

function AttendanceHistory({ records, loading, MapComponent }: { records: AttRecord[]; loading: boolean; MapComponent: ComponentType<AttendanceMapProps> | null }) {
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`font-mono text-sm ${complete ? "font-semibold text-foreground" : "text-muted-foreground"}`}>{w.checkOut || "—"}</span>
                          ...
                          <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[11px] text-muted-foreground">
                            ...
                            {hasGps ? (
                              acc ? (
                                <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider ${acc.cls}`}>{acc.txt}</span>
                              ) : (
                                <span className="rounded-full bg-emerald-500/10 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-400">GPS</span>
                              )
                            ) : (
                              <span className="rounded-full bg-secondary px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider text-muted-foreground">No GPS</span>
                            )}
                            {hasGps && (
                              <button
                                onClick={() => toggle(w.id)}
