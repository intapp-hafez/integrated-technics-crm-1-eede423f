...
function PipelinePage() {
  ...
  const employeeOptions = (() => {
    ...
    employees.forEach((e: any) => { if (e?.name) set.add(e.name); });
    return Array.from(set).sort((a, b) => a.localeCompare(b)).map((n) => ({ value: n, label: n }));
  })();
  ...
  const visibleLeads = employeeFilter.length === 0
    ? leads
    : leads.filter((l) => employeeFilter.includes(l.owner));
