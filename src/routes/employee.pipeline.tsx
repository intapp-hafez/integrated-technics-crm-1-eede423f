    ...
    <AppShell panel="employee" user={user} pageTitle={t("pipeline")}>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <MultiSelect label="Stage" options={stageOptions} selected={stageFilter} onChange={setStageFilter} />
          <div className="flex items-center gap-1">
            <Button variant="outline" size="icon" onClick={() => scrollBy(-1)} aria-label="Scroll stages left"><ChevronLeft /></Button>
            <Button variant="outline" size="icon" onClick={() => scrollBy(1)} aria-label="Scroll stages right"><ChevronRight /></Button>
          </div>
        </div>
      </div>
              ...
              onDrop={(e) => {
                ...
                if (lid) {
                  ...
                  if (lead && lead.status !== stage.key) {
                    setPending({ lead, toStage: stage.key as LeadStatus, toLabel: (t(stage.key as any) as string) ?? stage.label });
                  }
                }
                setDragId(null);
                setOverStage(null);
            ...
            >
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="rounded-full bg-background px-1.5 py-0.5 text-[10px] font-bold text-muted-foreground ring-1 ring-border">{stageLeads.length}</span>
                </div>
              ...
              <div className="min-h-[80px] space-y-2">
                {stageLeads.map((l) => (
                  <div
                    key={l.id}
                    draggable
                    onDragStart={(e) => { setDragId(l.id); e.dataTransfer.effectAllowed = "move"; e.dataTransfer.setData("text/lead-id", l.id); }}
                    onDragEnd={() => { setDragId(null); setOverStage(null); }}
                    onClick={() => { navigate({ to: "/employee/leads/$leadId", params: { leadId: l.id } }); }}
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <Link to="/employee/leads/$leadId" params={{ leadId: l.id }} onClick={(e) => e.stopPropagation()} className="text-muted-foreground opacity-0 transition group-hover:opacity-100 hover:text-primary" aria-label={t("openLead")}>
                        <ExternalLink className="h-3.5 w-3.5" />
                      </Link>
                    </div>
                    <div className="mt-3 flex items-center justify-between">
                      <span className="font-mono text-xs font-bold text-primary">{fmtMoney(l.value)}</span>
                      <div className="flex h-5 w-5 items-center justify-center rounded-full bg-primary/10 text-[9px] font-bold text-primary">{l.owner.split(" ").map((w: string) => w[0]).join("")}</div>
                    </div>
                ...
                {stageLeads.length === 0 && (
                  <div className="rounded-lg border border-dashed border-border py-6 text-center text-[11px] text-muted-foreground">{t("dropLeadsHere")}</div>
                )}
              </div>
            </div>
          );
