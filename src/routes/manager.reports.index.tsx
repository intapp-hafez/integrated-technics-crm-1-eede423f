...
function ManagerReportsPage() {
  ...
  const inactiveTeamLeadsCount = useMemo(() => {
  ...
  return (
    <AppShell
      ...
      pageTitle={t("reports")}
    >
      {/* Inactive Leads Quick Alert/Card */}
      <Link
        to="/manager/reports/inactive-leads"
        className="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-amber-500/30 bg-amber-500/10 p-5 transition hover:bg-amber-500/15 hover:border-amber-500/50 cursor-pointer block group"
      >
        <div className="flex items-center gap-3.5">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-amber-500/20 text-amber-600 dark:text-amber-400 group-hover:scale-105 transition-transform">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              {dir === "rtl"
                ? `يوجد ${inactiveTeamLeadsCount} عميل غير نشط في فريقك لم يتم التعامل معهم منذ 7 أيام أو أكثر.`
                : `There are ${inactiveTeamLeadsCount} inactive leads in your team with no activity for 7+ days.`}
            </div>
          </div>
        </div>
        <div className="inline-flex items-center gap-1.5 rounded-xl bg-amber-600 px-4 py-2 text-xs font-bold text-white shadow-sm transition group-hover:bg-amber-700">
          <span>{dir === "rtl" ? "عرض التقرير الكامل" : "View Full Report"}</span>
