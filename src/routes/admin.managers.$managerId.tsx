...
export const Route = createFileRoute("/admin/managers/$managerId")({
  component: ManagerDetailsPage,
  head: ({ params }) => ({ meta: [{ title: `${params.managerId} · INT-CRM` }] }),
});
...
function ManagerDetailsPage() {
  ...
  const { activities, history, employees, users, leads: storeLeads, attendance: storeAttendance, settings } = useStoreState();
  const workdayHours = settings.workdayHours ?? 8;
  // Managers live in `users` (auth role), not always in `employees` (which filters by role).
  // Try employees first (has perf data), then fall back to users store.
  const empFromEmployees = employees.find((e) => e.id === managerId);
  const empFromUsers = !empFromEmployees ? (users as any[]).find((u) => u.profileId === managerId || u.id === managerId) : undefined;
  const emp = empFromEmployees ?? (empFromUsers ? {
    id: empFromUsers.profileId ?? empFromUsers.id,
    userId: empFromUsers.id,
    name: empFromUsers.name,
    role: empFromUsers.titleEn || "Manager",
    department: empFromUsers.departmentEn || "—",
    perf: 0,
    leads: 0,
    won: 0,
    avatar: (empFromUsers.name as string).split(/\s+/).filter(Boolean).map((w: string) => w[0]?.toUpperCase() ?? "").join("").slice(0, 2) || "??",
    photo: empFromUsers.avatarUrl ?? "",
    email: empFromUsers.email ?? "",
    phone: empFromUsers.phone ?? "",
    annualTarget: empFromUsers.targetValue ?? 0,
    achievedTarget: 0,
  } : undefined) as any;
  ...
  const isActive = activeMap[managerId] !== false; // default true
  const toggleActive = () => {
    const next = { ...activeMap, [managerId]: !isActive };
    setActiveMap(next);
    saveActive(next);
  };
  ...
  const teamMembers = emp ? (employees as any[]).filter((e) => e.managerId === emp.id || e.managerId === (emp as any)?.profileId) : [];
  ...
  if (!emp) {
    return (
      <AppShell panel="admin" user={user} pageTitle="Manager">
        <div className="rounded-xl border border-dashed border-border bg-card p-12 text-center">
          <p className="text-sm text-muted-foreground">Manager <span className="font-mono">{managerId}</span> not found.</p>
          <Link to="/admin/managers" className="mt-3 inline-block text-sm font-semibold text-primary">{t("backToManagers")}</Link>
        </div>
      </AppShell>
    );
  }
  ...
  return (
    <AppShell panel="admin" user={user} pageTitle={emp.name}>
      <button onClick={() => router.history.back()} className="mb-4 inline-flex items-center gap-1.5 text-sm font-semibold text-muted-foreground hover:text-primary">
        <ArrowLeft className="h-4 w-4" /> {t("backToManagers")}
      </button>
      ...
      {tab === "overview" && (
        ...
        <div className="mt-6 rounded-2xl border border-border bg-card p-5 shadow-[var(--shadow-soft)] lg:col-span-2">
          <div className="mb-4 flex items-center gap-2">
            <Users2 className="h-4 w-4 text-primary" />
            <h3 className="font-display text-sm font-bold uppercase tracking-wider text-foreground">{t("teamMembers") ?? "Team Members"} ({teamMembers.length})</h3>
          </div>
          {teamMembers.length === 0 && <p className="text-sm text-muted-foreground">No employees report to this manager.</p>}
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            {teamMembers.map((member) => (
              <Link 
                ...
                className="flex items-center gap-3 rounded-xl border border-border p-3 transition hover:-translate-y-0.5 hover:border-primary hover:shadow-md"
              >
                {member.photo ? (
                  <img src={member.photo} alt={member.name} className="h-10 w-10 shrink-0 rounded-full object-cover ring-1 ring-primary/30" />
                ) : (
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-muted-foreground truncate">{member.role} {member.department ? `· ${member.department}` : ''}</div>
                </div>
                <div className="text-end shrink-0">
                  <div className={`font-mono text-xs font-bold ${member.perf >= 100 ? "text-emerald-600" : member.perf >= 75 ? "text-amber-600" : "text-rose-600"}`}>
                    {member.perf}%
                  </div>
                  <div className="text-[9px] uppercase text-muted-foreground">{t("performance")}</div>
                </div>
              </Link>
            ))}
          </div>
