...
function ManagerDetailsPage() {
  ...
  const empFromUsers = !empFromEmployees
    ? (users as any[]).find((u) => u.profileId === managerId || u.id === managerId)
    : undefined;
  const emp =
    empFromEmployees ??
    ((empFromUsers
  ...
  const attendanceRate =
    workingDays > 0 ? Math.min(100, ((presentDays + lateDays * 0.5) / workingDays) * 100) : 91;
      ...
      <AppShell panel="admin" user={user} pageTitle="Manager">
        <div className="rounded-xl border border-dashed border-border bg-card p-12 text-center">
          <p className="text-sm text-muted-foreground">
            Manager <span className="font-mono">{managerId}</span> not found.
          </p>
          <Link
            to="/admin/managers"
            ...
            {displayedAccounts.map((p) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {(p as any).contactName || p.client} {p.category ? `· ${p.category}` : ""}
                    {(p as any).contactName && p.client && (p as any).contactName !== p.client && (
      ...
      {tab === "team" && (
        <div className="mt-6 rounded-2xl border border-border bg-card p-5 shadow-[var(--shadow-soft)] lg:col-span-2">
          <div className="mb-4 flex items-center gap-2">
            <Users2 className="h-4 w-4 text-primary" />
            <h3 className="font-display text-sm font-bold uppercase tracking-wider text-foreground">
              {t("teamMembers") ?? "Team Members"} ({teamMembers.length})
            </h3>
          </div>
          {teamMembers.length === 0 && (
            <p className="text-sm text-muted-foreground">No employees report to this manager.</p>
          )}
              ...
              >
                {member.photo ? (
                  <img
                    src={member.photo}
                    alt={member.name}
                    className="h-10 w-10 shrink-0 rounded-full object-cover ring-1 ring-primary/30"
                  />
                ) : (
                ...
                <div className="text-end shrink-0">
                  <div
                    className={`font-mono text-xs font-bold ${member.perf >= 100 ? "text-emerald-600" : member.perf >= 75 ? "text-amber-600" : "text-rose-600"}`}
                  >
                    {member.perf}%
                  </div>
