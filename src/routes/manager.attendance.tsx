...
function ManagerAttendancePage() {
  ...
  const stats = [
    { label: t("presentToday"), v: present, total, Icon: CheckCircle2, color: "text-emerald-600", bg: "bg-emerald-50" },
    { label: t("late"), v: late, total, Icon: AlertTriangle, color: "text-amber-600", bg: "bg-amber-50" },
    { label: t("absent"), v: absent, total, Icon: XCircle, color: "text-rose-600", bg: "bg-rose-50" },
  ];

  return (
    <AppShell panel="manager" user={{ name: "hafez Rahim", role: t("manager"), initials: "HR", photo: "https://cdn.pixabay.com/photo/2022/03/11/06/14/indian-man-7061278_1280.jpg" }} pageTitle={t("attendance")}>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {todayRecords.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-10 text-center text-sm text-muted-foreground">—</td></tr>
              )}
              {todayRecords.map((r) => {
                  ...
                  <tr key={r.id} className="hover:bg-primary/5">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        {emp?.photo ? (
                          <img src={emp.photo} alt={r.owner} loading="lazy" className="h-8 w-8 shrink-0 rounded-full object-cover ring-1 ring-primary/30" />
                        ) : (
                          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-primary to-orange-600 text-[10px] font-bold text-primary-foreground">
                            {emp?.avatar ?? r.owner.split(" ").map(w => w[0]).join("").slice(0, 2)}
                          </div>
                        )}
                    ...
                    <td className="px-4 py-3 font-mono text-muted-foreground">{r.checkOut || "—"}</td>
                    <td className="px-4 py-3 text-muted-foreground">
                      <div className="flex items-center gap-1.5">
                        <MapPin className="h-3 w-3" /> {r.location}
                      </div>
                    </td>
                    <td className="px-4 py-3"><StatusBadge status={status} label={t(status as any)} /></td>
                  </tr>
                );
              })}
            </tbody>
