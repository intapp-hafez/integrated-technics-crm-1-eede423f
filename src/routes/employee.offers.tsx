...
function EmployeeOffersPage() {
  ...
  const isDetail = useRouterState({
    select: (s) => s.location.pathname.startsWith("/employee/offers/"),
  });
  ...
  return (
    <AppShell
      panel="employee"
      user={{ name: OWNER, role: t("employee"), initials, photo: profile.avatarUrl }}
      pageTitle={t("offers")}
    >
              ...
              {filtered.map((q) => (
                <tr key={q.id} className="cursor-pointer transition hover:bg-primary/5">
                  <td className="px-4 py-3 font-mono text-xs text-muted-foreground">
                    <Link
                      to="/employee/offers/$quotationId"
