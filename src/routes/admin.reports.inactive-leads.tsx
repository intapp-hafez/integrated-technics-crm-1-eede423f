...
export const Route = createFileRoute("/admin/reports/inactive-leads")({
  component: InactiveLeadsReportPage,
  head: () => ({ meta: [{ title: "Inactive Leads Report · INT-CRM" }] }),
});
  ...
  const handleAction = async (lead: any, type: "reminder" | "warning") => {
    ...
    if (email !== "unknown") {
      ...
      try {
        ...
        toast.error(
          (isAr ? "فشل إرسال البريد الإلكتروني: " : "Failed to send email: ") +
            (error.message || ""),
        );
      }
    } else {
