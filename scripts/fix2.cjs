...
walkSync('src', filePath => {
  ...
  content = content.replace(/\{l\.expectedCloseDate \|\| ("—"|'—')\}/g, '{formatDate(l.expectedCloseDate)}');
  content = content.replace(/\{\(lead as any\)\.expectedCloseDate \|\| ("—"|'—')\}/g, '{formatDate((lead as any).expectedCloseDate)}');
  content = content.replace(/\['Expected Close',\s*\(lead as any\)\.expectedCloseDate \|\| '—'\],/g, `['Expected Close', formatDate((lead as any).expectedCloseDate)],`);
